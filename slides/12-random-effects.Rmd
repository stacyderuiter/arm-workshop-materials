...
- But don't want to (or can't) add another "regular" predictor.
...
s245::gf_acf(~base_model)
# or use: acf(resid(base_model))
```
...
---
## Random vs. Fixed Effect Predictors

Random effect variables...

.small[
- Must be categorical
- Plays a role suggesting we should include it as a predictor (confounder, precision covariate, ...)
- Often measure time, place, or individual ID
- Often have *many* possible values of which we observe only a subset (so we'd rather not hav to specify *one* to predict for)
- May have too many values to include as fixed effect (dataset size)
- Probably, we don't want to make inference about it
]

### In your work?
---
# Nesting?
### A student is nested in a class, in a school, in a district, in a state, in a country...

---

# Random effect candidates? 

<https://stacyderuiter.github.io/stat245-sp25/which-re-variable.pdf>
...
## Model Comparison/Inference for Mixed Models
...
.pull-left[
### ML
- RE variance estimates may be *biased* if $n$ is too small
- ANOVA or AIC/BIC may be used to compare models with different fixed effects
]

.pull-right[
### REML
- unbiased estimates of RE variance parameters
- *Cannot* use ANOVA or AIC/BIC to compare models with different fixed effects
...
- [`glmmTMB` troubleshooting](https://glmmtmb.github.io/glmmTMB/articles/troubleshooting.html) help online
- or stack overflow? (thanks, [Ben](https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://www.math.mcmaster.ca/bolker))
...
- Do we want to make predictions for "the typical random effect grouping" (here, the average whale in the average time-block)?
- Or do we want to make predictions *averaged across* a whole population of random effect groups (all the whales at all times)?
- For a *linear* model, *the two are numerically equal* but for models with a link function *they are different and we have to choose and know which we are presenting.*
